...
The ADMM algorithm solves problems of the form min f(x)+g(z), subject to Ax+Bz=C. The core idea is decomposition optimization, which treats the augmented Lagrange function as a function of two variables x and z, and optimizes x with fixed z and λ at each step, and optimizes z with fixed x and λ at each step, and introduces a quadratic penalty term to guarantee convergence. 
...
Under this function, the expected return and variance of the portfolio can be calculated using the given asset return, covariance matrix and weight, so as to carry out effective portfolio optimization. 
